...
*Please not that in some pictures you might find a reference to eurodeer_db instead of afrimove_db. All the examples work perfectly once you replace eurodeer with afrimove as (at the moment) they are hosted on the same server.*

[PostgreSQL](https://www.postgresql.org/) is the open source database server platform used by AfriMove in combination with the spatial extension [PostGIS](https://www.postgis.org/) (The role of the database server is to distribute data to client applications keeping the data into a centralized repository. Many different interfaces can be used. Some are focused on the creation, maintenance and development of the database, others to visualize and edit/manipulate tabular or spatial data. Statistical tools can be used to perform analysis on the data. Many graphical interface can be used to offer data in a user-friendly fashion, including through web applications.  
...
AFRIMOVE db is based on PostgreSQL 9.5 and PostGIS 9.5.
